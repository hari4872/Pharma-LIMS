...
public class LaboratoriesController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateLaboratoryRequest request)
    {
        ...
        var result = await _mediator.Send(new CreateLaboratoryCommand(request.LabName, request.Site ?? string.Empty, request.Location, request.LabType, username));
...
public record CreateLaboratoryRequest(string LabName, string Location, string LabType, string? Site = null);
public record UpdateLaboratoryRequest(string LabName, string Location, string LabType, string? Site = null);
