...
public class ApproveCoAHandler : IRequestHandler<ApproveCoACommand, Result<int>>
{
    ...
    public async Task<Result<int>> Handle(ApproveCoACommand cmd, CancellationToken ct)
    {
        ...
        // 10-item QA checklist — all must pass (vw_qa_checklist equivalent — Contract 2)
        var checklist = await _qaGate.EvaluateChecklistAsync(coa.SampleId, cmd.CoaId, ct);
        if (!checklist.AllPassed)
        {
            var failed = new List<string>();
            if (!checklist.TestsComplete)       failed.Add("Item 1: Not all test executions complete");
            ...
            if (!checklist.CorrectSpecVersion)  failed.Add("Item 7: Incorrect or unapproved spec version used");
        ...
        coa.Sample.Status = SampleStatus.Released;
