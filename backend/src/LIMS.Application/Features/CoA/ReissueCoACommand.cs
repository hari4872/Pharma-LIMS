...
public record ReissueCoACommand(int OriginalCoaId, string Reason) : IRequest<Result<int>>;
...
public class ReissueCoAHandler : IRequestHandler<ReissueCoACommand, Result<int>>
{
    ...
    public ReissueCoAHandler(ILimsDbContext db, IMasterDataAuditService audit)
    { _db = db; _audit = audit; }
    ...
    public async Task<Result<int>> Handle(ReissueCoACommand cmd, CancellationToken ct)
    {
        ...
        var config = await _db.LabConfigs
            .FirstOrDefaultAsync(c => c.ConfigKey == "coa_number_format", ct);
        ...
        await _audit.LogAsync("CoA", newCoa.CoaId, "Reissued",
            ...
            new { NewCoaId = newCoa.CoaId, newCoa.CoaNumber, cmd.Reason },
            "System");
