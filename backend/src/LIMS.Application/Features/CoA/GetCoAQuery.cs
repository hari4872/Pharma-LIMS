...
public record GetCoAQuery(int? SampleId, string? Status) : IRequest<List<CoADto>>;
...
public class GetCoAHandler : IRequestHandler<GetCoAQuery, List<CoADto>>
{
    ...
    public async Task<List<CoADto>> Handle(GetCoAQuery q, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(q.Status)) query = query.Where(c => c.Status.ToString() == q.Status);
